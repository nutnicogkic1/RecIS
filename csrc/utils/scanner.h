...
class Scanner {
 public:
  ...
  explicit Scanner(torch::string_view source) : cur_(source) {
    RestartCapture();
  }
  ...
  Scanner &ZeroOrOneLiteral(torch::string_view s) {
    ConsumePrefix(cur_, s);
    return *this;
  }

  // Consume the next s.size() characters of the input, if they match <s>. If
  // they don't match <s>, then GetResult will ultimately return false.
  Scanner &OneLiteral(torch::string_view s) {
    if (!ConsumePrefix(cur_, s)) {
      error_ = true;
    }
    return *this;
  }
  ...
  bool GetResult(torch::string_view *remaining = nullptr,
                 torch::string_view *capture = nullptr);
