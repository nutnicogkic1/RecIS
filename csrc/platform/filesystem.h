...
class FileSystem {
 public:
  ...
  virtual Status NewTransactionFile(const std::string &fname,
                                    std::unique_ptr<WritableFile> *result);

  /// \brief Overwrites the target if it exists.
  virtual Status TransactionRenameFile(const std::string &src,
...
class RandomAccessFile {
 public:
  ...
  virtual Status Read(uint64_t offset, size_t n, torch::string_view *result,
                      char *scratch) const = 0;

 private:
  AT_DISALLOW_COPY_AND_ASSIGN(RandomAccessFile);
};
...
class WritableFile {
 public:
  ...
  virtual Status Append(torch::string_view data) = 0;
 ...
 private:
  AT_DISALLOW_COPY_AND_ASSIGN(WritableFile);
};
