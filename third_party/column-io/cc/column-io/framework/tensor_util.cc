...
Status Encode(const Tensor &tensor, TensorProto *proto) {
  ...
  if (tensor.Shape().IsScalar()) {
    proto->mutable_tensor_shape()->set_is_scalar(true);
    proto->mutable_tensor_shape()->set_num_elements(1);
  } else {
    proto->mutable_tensor_shape()->set_is_scalar(false);
    proto->mutable_tensor_shape()->set_num_elements(0);
  }
  ...
  content->assign(reinterpret_cast<const char *>(ptr), bytes);
  return Status::OK();
}

Status DecodeAndMake(const TensorProto &proto, Tensor *tensor) {
  if (proto.tensor_shape().is_scalar()) {
    *tensor = Tensor(static_cast<DataType>(proto.dtype()));
  } else {
    std::vector<size_t> dims(proto.tensor_shape().dims().begin(),
                             proto.tensor_shape().dims().end());
    *tensor = Tensor(static_cast<DataType>(proto.dtype()), dims);
  }
...
TensorShape ProtoToTensorShape(const TensorShapeProto &proto) {
  ...
  std::copy(proto.dims().begin(), proto.dims().end(), dims.begin());
