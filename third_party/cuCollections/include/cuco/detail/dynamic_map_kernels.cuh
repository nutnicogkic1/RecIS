 ...
 * @brief Inserts all key/value pairs in the range `[first, last)`.
 *
 * @note This function is unsafe. caller must ensure that the keys in `[first, last)` are
...
template <uint32_t block_size,
          ...
          typename Hash,
          typename KeyEqual>
CUCO_KERNEL void insert_unsafe(InputIt first,
                               InputIt last,
                               viewT* submap_views,
                               mutableViewT* submap_mutable_views,
                               atomicT** submap_num_successes,
...
template <uint32_t block_size,
          ...
          typename Hash,
          typename KeyEqual>
CUCO_KERNEL void insert_key_value_unsafe(KeyIt key_first,
                                         KeyIt key_last,
                                         ValueIt value_first,
                                         viewT* submap_views,
                                         mutableViewT* submap_mutable_views,
                                         ...
                                         KeyEqual key_equal)
{
  ...
  auto key_it   = key_first + idx;
  auto value_it = value_first + idx;

  while (key_it < key_last) {
    pair_type insert_pair = pair_type{*key_it, *value_it};
    if (submap_mutable_views[insert_idx].insert(tile, insert_pair, hash, key_equal) &&
        tile.thread_rank() == 0) {
      thread_num_successes++;
    }

    key_it += (gridDim.x * blockDim.x) / tile_size;
    value_it += (gridDim.x * blockDim.x) / tile_size;
  }
...
template <uint32_t block_size,
          ...
          typename Hash,
          typename KeyEqual>
CUCO_KERNEL void find_value_and_mask(InputIt first,
                                     InputIt last,
                                     ...
                                     KeyEqual key_equal)
{
  auto tid                  = blockDim.x * blockIdx.x + threadIdx.x;
  auto empty_value_sentinel = submap_views[0].get_empty_value_sentinel();
  __shared__ Value valueBuffer[block_size];
  ...
  while (first + tid < last) {
    ...
    for (auto i = 0; i < num_submaps; ++i) {
      auto submap_view = submap_views[i];
      submap_view.find(key, found_value, not_exist, hash, key_equal);
      if (!cuco::detail::bitwise_compare(found_value, empty_value_sentinel)) {
        break;
      }
    }
     ...
     * L2 to global and improving performance.
     */
    valueBuffer[threadIdx.x] = found_value;
    maskBuffer[threadIdx.x] = not_exist;
    __syncthreads();
    *(output_begin + tid) = valueBuffer[threadIdx.x];
    *(mask_begin + tid) = maskBuffer[threadIdx.x];
    tid += gridDim.x * blockDim.x;
  }
}
/**
                              ...
                              size_t* cap_prefix_sum)
{
  ...
  while ((tid - threadIdx.x) < range) {
    uint32_t submap_idx    = 0;
    uint32_t submap_offset = tid + start;
    bool is_filled = false;
    Key existing_key;
    Value existing_value;
    if (tid < range) {
    while (submap_offset >= cap_prefix_sum[submap_idx] && submap_idx < num_submaps) {
      ++submap_idx;
    }
    if (submap_idx > 0) { submap_offset = tid + start - cap_prefix_sum[submap_idx - 1]; }

    auto const& current_slot = submap_views[submap_idx].get_slots()[submap_offset];
    existing_key = current_slot.first.load(cuda::std::memory_order_relaxed);
    existing_value = current_slot.second.load(cuda::std::memory_order_relaxed);

    is_filled = not(cuco::detail::bitwise_compare(existing_key, empty_key_sentinel) or
                               cuco::detail::bitwise_compare(existing_key, erased_key_sentinel));
    }
    unsigned int local_idx   = 0;
    unsigned int block_valid = 0;
    ...
    if (is_filled) {
      keys_out[block_base + local_idx] = existing_key;
      values_out[block_base + local_idx] = existing_value;
    }
    tid += gridDim.x * blockDim.x;
  }
}
