...
{
  auto current_slot_init{initial_slot(k, hash)};
  auto current_slot = current_slot_init;
  ...
  while (true) {
    ...
    current_slot = next_slot(current_slot);
    if (current_slot_init == current_slot) {
      return false;
    }
  }
}
...
{
  auto current_slot_init = initial_slot(g, k, hash);
  auto current_slot = current_slot_init;
  value_type const insert_pair =
    make_pair<Key, Value>(this->get_erased_key_sentinel(), this->get_empty_value_sentinel());
                                                            ...
                                                            KeyEqual key_equal) noexcept
{
  ...
  while (true) {
    ...
    // Key doesn't exist
    if (cuco::detail::bitwise_compare(existing_key, this->get_empty_key_sentinel())) {
      v = this->get_empty_value_sentinel();
      not_exist = true;
      return;
    }
    ...
    // Key exists
    if (key_equal(existing_key, k)) {
      v = current_slot->second.load(cuda::std::memory_order_relaxed);
      not_exist = false;
      return;
    }
